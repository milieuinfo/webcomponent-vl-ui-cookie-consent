import {VlElement, define} from '/node_modules/vl-ui-core/dist/vl-core.min.js'; import '/node_modules/vl-ui-button/dist/vl-button.min.js'; import '/node_modules/vl-ui-form-grid/dist/vl-form-grid.min.js'; import '/node_modules/vl-ui-modal/dist/vl-modal.min.js'; import {analytics} from '/node_modules/vl-ui-cookie-consent/src/analytics.js'; import '/node_modules/vl-ui-cookie-consent/dist/vl-cookie-consent-opt-in.min.js'; export class VlCookieConsent extends (VlElement(HTMLElement)) {
  static get _observedAttributes() {
    ...
    const e=Object.values(this._optIns).length>0?'Bewaar keuze':'Ik begrijp het'; const t=this._template(`\n            <button is="vl-button" slot="button">${e}</button>\n        `); return t.querySelector('button').addEventListener('click', ()=>{
      this.close();
    }), t;
  }_getOptInTemplate(e) {
    if (e) {
      const t=e.value||e.mandatory?VlCookieConsent.attributePrefix+'checked':''; const n=e.mandatory?VlCookieConsent.attributePrefix+'mandatory':''; const l=this._template(`\n                <div is="vl-form-column">\n                    <vl-cookie-consent-opt-in ${VlCookieConsent.attributePrefix}label="${e.label}" ${VlCookieConsent.attributePrefix}description="${e.description}" ${t} ${n}></vl-cookie-consent-opt-in>\n                </div>\n            `); return l.querySelector('vl-cookie-consent-opt-in').addEventListener('input', (t)=>{
        const n=!(!t||!t.currentTarget)&&t.currentTarget.checked; e.value=n;
      }), l;
    }
  }_open(e) {
    !e&&this._getCookieConsentCookie()&&this._heeftCookieConsentDateCookie()&&this._isCookieConsentCookieGeldig()||this._modalElement.open();
  }_resetOptInValue(e) {
    [...this._optInElementen].find((e)=>e.id=e.name)&&(e.value=null!=e.getAttribute(VlCookieConsent.attributePrefix+'checked'));
  }_processOptIns() {
    this._optInElementen.forEach((e)=>{
      this._processOptIn({name: e.id, label: e.getAttribute(VlCookieConsent.attributePrefix+'label'), description: e.getAttribute(VlCookieConsent.attributePrefix+'description'), value: null!=e.getAttribute(VlCookieConsent.attributePrefix+'checked'), mandatory: null!=e.getAttribute(VlCookieConsent.attributePrefix+'mandatory')});
    });
  }_processOptIn({name: e, label: t, description: n, value: l, mandatory: r, callback: {activated: o, deactivated: i}={}}) {
    if (!this._bevatOptIn(e)) {
      const a=this._getCookie(e); const v=this._optIns[e]={name: e, label: t, description: n, value: void 0!==a?a:l, callback: {activated: o, deactivated: i}, mandatory: !!r}; const f=this._getOptInTemplate(v); f&&this._formGridElement.appendChild(f);
    }
  }_submitOptIns() {
    Object.values(this._optIns).forEach((e)=>{
      e.callback&&(e.value||e.mandatory?e.callback.activated&&e.callback.activated():e.callback.deactivated&&e.callback.deactivated()), this._setCookie(e.name, e.value||e.mandatory||!1);
    });
  }_bevatOptIn(e) {
    return !!this._optIns[e];
  }_addFunctionalOptIn() {
    this._processOptIn({name: 'functional', label: 'Noodzakelijke cookies toestaan (verplicht)', description: 'Noodzakelijke cookies helpen een website bruikbaarder te maken, door basisfuncties als paginanavigatie en toegang tot beveiligde gedeelten van de website mogelijk te maken. Zonder deze cookies kan de website niet naar behoren werken.', value: !0, mandatory: !0});
  }_addAnalytics() {
    document.getElementById(analytics.scriptId)||document.head.appendChild(analytics.script);
  }_getCookie(e) {
    e=this._cookiePrefix+e+'='; const t=document.cookie.split(';'); for (let n=0; n<t.length; n++) {
      for (var l=t[n]; ' '==l.charAt(0);)l=l.substring(1); if (0==l.indexOf(e)) {
        try {
          return JSON.parse(l.substring(e.length, l.length));
        } catch (t) {
    ...
    document.cookie=this._cookiePrefix+e+'='+t+';Max-Age=2147483647;path=/;';
  }_deleteCookie(e) {
    document.cookie=this._cookiePrefix+e+'=;expires=Thu, 01 Jan 1970 00:00:00 UTC;path=/;';
  }_heeftCookieConsentDateCookie() {
    return null!=this._getCookieConsentDateCookie();
  ...
  }_analyticsChangedCallback(e, t) {
    null!=t&&(this._isFunctionalOptInDisabled?console.error('analytics kunnen alleen toegevoegd worden wanneer de functionele cookies opt-in geactiveerd werd!'):this._addAnalytics());
  }_ownerChangedCallback(e, t) {
    this._ownerElements.forEach((e)=>e.innerText=t);
  }_linkChangedCallback(e, t) {
    this._linkElement.innerText=t, this._linkElement.href=t;
  }
